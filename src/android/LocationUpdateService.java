...
public class LocationUpdateService extends Service implements LocationListener {
	...
	private static final long STATIONARY_LOCATION_POLLING_INTERVAL_LAZY = 3 * 1000 * 60; // 3
																							// minutes.
	private static final long STATIONARY_LOCATION_POLLING_INTERVAL_AGGRESSIVE = 1 * 1000 * 60; // 1
																								// minute.
	...
	public void onCreate() {
		...
		// Stop-detection PI
		stationaryAlarmPI = PendingIntent.getBroadcast(this, 0, new Intent(
				STATIONARY_ALARM_ACTION), 0);
		registerReceiver(stationaryAlarmReceiver, new IntentFilter(
				STATIONARY_ALARM_ACTION));

		// Stationary region PI
		stationaryRegionPI = PendingIntent.getBroadcast(this, 0, new Intent(
				STATIONARY_REGION_ACTION), PendingIntent.FLAG_CANCEL_CURRENT);
		registerReceiver(stationaryRegionReceiver, new IntentFilter(
		...
		registerReceiver(stationaryLocationMonitorReceiver, new IntentFilter(
				STATIONARY_LOCATION_MONITOR_ACTION));

		// One-shot PI (TODO currently unused)
		singleUpdatePI = PendingIntent.getBroadcast(this, 0, new Intent(
				SINGLE_LOCATION_UPDATE_ACTION),
				PendingIntent.FLAG_CANCEL_CURRENT);
		registerReceiver(singleUpdateReceiver, new IntentFilter(
				SINGLE_LOCATION_UPDATE_ACTION));
	...
	@TargetApi(15)
	private Notification buildForegroundNotificationCompat(
			Notification.Builder builder) {
	...
	public boolean stopService(Intent intent) {
		...
		cleanUp();
		if (isDebugging) {
			Toast.makeText(this, "Background location tracking stopped",
	 ...
	 */
	private void onPaceChange(Boolean isMoving) {
		Log.i(TAG, "Pace changed - moving: " + isMoving);
		setPace(isMoving);
	}
	 ...
	 * 
	 * @param value
	 *            set true to engage "aggressive", battery-consuming tracking,
	...
	private void setPace(Boolean value) {
		...
		// Temporarily turn on super-aggressive geolocation on all providers
		// when acquiring velocity or stationary location.
	...
	/**
	 * Translates a number representing desired accuracy of GeoLocation system
	 * from set [0, 10, 100, 1000]. 0: most aggressive, most accurate, worst
	 * battery drain 1000: least aggressive, least accurate, best for battery.
	 */
	...
	public Location getLastBestLocation() {
		...
		Log.i(TAG, "- fetching last best location " + minDistance + ","
				+ minTime);
		...
		for (String provider : matchingProviders) {
			...
			if (location != null) {
				...
				Log.d(TAG, "time>minTime: " + (time > minTime)
	...
	public void onLocationChanged(Location location) {
		Log.d(TAG,
				"- onLocationChanged: " + location.getLatitude() + ","
						...
						+ location.getAccuracy() + ", isMoving: " + isMoving
						+ ", speed: " + location.getSpeed());

		if (!isMoving && !isAcquiringStationaryLocation
				&& stationaryLocation == null) {
			// Perhaps our GPS signal was interupted, re-acquire a
			// stationaryLocation now.
			setPace(false);
		}
		...
		if (isDebugging) {
			Toast.makeText(
					this,
					"mv:" + isMoving + ",acy:" + location.getAccuracy() + ",v:"
							+ location.getSpeed() + ",df:"
							+ scaledDistanceFilter, Toast.LENGTH_LONG).show();
		}
		if (isAcquiringStationaryLocation) {
			if (stationaryLocation == null
					|| stationaryLocation.getAccuracy() > location
							.getAccuracy()) {
			...
			if (++locationAcquisitionAttempts == MAX_STATIONARY_ACQUISITION_ATTEMPTS) {
				...
				// Unacceptable stationary-location: bail-out and wait for
				// another.
				if (isDebugging) {
					startTone("beep");
				}
			...
			if (++locationAcquisitionAttempts == MAX_SPEED_ACQUISITION_ATTEMPTS) {
				// Got enough samples, assume we're confident in reported speed
				// now. Play "woohoo" sound.
				if (isDebugging) {
					startTone("doodly_doo");
				}
				isAcquiringSpeed = false;
				scaledDistanceFilter = calculateDistanceFilter(location
						.getSpeed());
				setPace(true);
			} else {
				if (isDebugging) {
			...
			// Only reset stationaryAlarm when accurate speed is detected,
			// prevents spurious locations from resetting when stopped.
			if ((location.getSpeed() >= 1)
					&& (location.getAccuracy() <= stationaryRadius)) {
				resetStationaryAlarm();
			}
			// Calculate latest distanceFilter, if it changed by 5 m/s, we'll
			// reconfigure our pace.
			Integer newDistanceFilter = calculateDistanceFilter(location
					.getSpeed());
			if (newDistanceFilter != scaledDistanceFilter.intValue()) {
				Log.i(TAG, "- updated distanceFilter, new: "
						+ newDistanceFilter + ", old: " + scaledDistanceFilter);
	...
	public void resetStationaryAlarm() {
		...
		alarmManager.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis()
				+ STATIONARY_TIMEOUT, stationaryAlarmPI); // Millisec * Second *
															// Minute
	}
	...
	private Integer calculateDistanceFilter(Float speed) {
		...
		if (speed < 100) {
			...
			newDistanceFilter = pow(roundedDistanceFilter, 2)
					+ (double) distanceFilter;
		}
		return (newDistanceFilter.intValue() < 1000) ? newDistanceFilter
				.intValue() : 1000;
	}
	...
	private void startMonitoringStationaryRegion(Location location) {
		...
		Log.i(TAG,
				"- startMonitoringStationaryRegion (" + location.getLatitude()
						+ "," + location.getLongitude() + "), accuracy:"
						+ location.getAccuracy());
		...
		locationManager
				.addProximityAlert(
						...
						location.getLongitude(),
						(location.getAccuracy() < stationaryRadius) ? stationaryRadius
								: location.getAccuracy(), (long) -1,
						stationaryRegionPI);
	...
	public void startPollingStationaryLocation(long interval) {
		// proximity-alerts don't seem to work while suspended in latest Android
		// 4.42 (works in 4.03). Have to use AlarmManager to sample
		...
		alarmManager.setInexactRepeating(AlarmManager.RTC_WAKEUP, start,
				interval, stationaryLocationPollingPI);
	}
	...
	public void onPollStationaryLocation(Location location) {
		...
		float distance = abs(location.distanceTo(stationaryLocation)
				- stationaryLocation.getAccuracy() - location.getAccuracy());

		if (isDebugging) {
			Toast.makeText(
					this,
					"Stationary exit in " + (stationaryRadius - distance) + "m",
	...
	public void onExitStationaryRegion(Location location) {
		// Filter-out spurious region-exits: must have at least a little speed
		// to move out of stationary-region
		if (isDebugging) {
	...
	/**
	 * TODO Experimental cell-tower change system; something like ios
	 * significant changes.
	 */
	public void onCellLocationChange(CellLocation cellLocation) {
		Log.i(TAG, "- onCellLocationChange" + cellLocation.toString());
		if (isDebugging) {
			Toast.makeText(this, "Cellular location change", Toast.LENGTH_LONG)
					.show();
	...
	/**
	 * Broadcast receiver which detcts a user has stopped for a long enough time
	 * to be determined as STOPPED
	 */
	...
	/**
	 * Broadcast receiver to handle stationaryMonitor alarm, fired at low
	 * frequency while monitoring stationary-region. This is required because
	 * latest Android proximity-alerts don't seem to operate while suspended.
	 * Regularly polling the location seems to trigger the proximity-alerts
	 * while suspended.
	 */
	...
	private BroadcastReceiver stationaryRegionReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (entering) {
				...
				Log.d(TAG, "- EXIT");
				// There MUST be a valid, recent location if this event-handler
				// was called.
	...
	private boolean postLocation(
			com.tenforwardconsulting.cordova.bgloc.data.Location l,
			LocationDAO dao) {
		if (l == null) {
		...
		try {
			...
			while (headkeys.hasNext()) {
				...
				if (headkey != null) {
					Log.d(TAG, "Adding Header: " + headkey + " : "
							+ (String) headers.getString(headkey));
					request.setHeader(headkey,
							(String) headers.getString(headkey));
				}
			}
	...
	private void persistLocation(Location location) {
		LocationDAO dao = DAOFactory.createLocationDAO(this
				.getApplicationContext());
		com.tenforwardconsulting.cordova.bgloc.data.Location savedLocation = com.tenforwardconsulting.cordova.bgloc.data.Location
	...
	private void cleanUp() {
		...
		if (stationaryLocation != null && !isMoving) {
			try {
				...
				Log.w(TAG,
						"- Something bad happened while removing proximity-alert");
			}
		}
		stopForeground(true);
		wakeLock.release();
	...
	private class PostLocationTask extends AsyncTask<Object, Integer, Boolean> {
		...
		protected Boolean doInBackground(Object... objects) {
			...
			LocationDAO locationDAO = DAOFactory
					.createLocationDAO(LocationUpdateService.this
							.getApplicationContext());
			for (com.tenforwardconsulting.cordova.bgloc.data.Location savedLocation : locationDAO
					.getAllLocations()) {
